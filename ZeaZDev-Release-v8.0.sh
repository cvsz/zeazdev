...
# 📦 Version: Release — v8.0 (The Standardization Upgrade)
...
# 🔐 License: MIT
# =============================================================================
# UPGRADE v8.0 (Critical Fix & Standardization):
# 1. [FEAT] (Display) Added ASCII Art display on install.
# 2. [FEAT] (Non-Interactive) Removed all 'read -p' prompts and added ENV validation.
# 3. [FEAT] (Health Check) Integrated API Health Check as final validation step.
# 4. [FIX] (Node.js) Installer now FORCES Node.js 22.x (LTS) (setup_22.x).
# 5. [FIX] (Hardhat) FORCED Ethers v5 path (Hardhat 2.21.1 / Toolbox 3.0.0).
# 6. [FIX] (Hardhat) REMOVED Ethers v6 logic and deploy-v6.js script.
# 7. [FIX] (Hardhat) REMOVED 'Auto-Heal HH801' step (v7.4).
# 8. [FIX] (Frontend) Fixed IDKit build error ("./hooks/useIDKit") by adding
#    'alias' entries to the Frontend's package.json.
# =============================================================================
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v8.0 (Standardization Upgrade)"
# --- [END FEAT] ---
...
if [ -z "$MULTI_RPC_LIST" ] || [ -z "$WORLD_APP_ID" ] || [ -z "$PRIVATE_KEY" ]; then
  err "Required ENV variables are missing."
  err "Please export: MULTI_RPC_LIST, WORLD_APP_ID, PRIVATE_KEY"
  exit 1
fi
...
/* 🌐 ZeaZDev v8.0 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v8.0 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v8.0 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v8.0 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
async function main(){
  ...
  await zea.transfer(airdropAddr, hre.ethers.utils.parseUnits("500000000", 18)); // v5 syntax
  await zea.transfer(rewardAddr, hre.ethers.utils.parseUnits("100000000", 18)); // v5 syntax
  const result = {
...
/* 🌐 ZeaZDev v8.0 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v8.0 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v8.0)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v8.0 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v8.0 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v8.0 $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v8.0 (Standardized) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v8.0 (Standardization Upgrade) installation complete!"
