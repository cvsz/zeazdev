...
# 📦 Version: Release — v7.3 (Hotfix 13)
...
# 🔐 License: MIT
# =============================================================================
# HOTFIX v7.3 (Critical Dependency Fix):
# 1. [FIX] (Hardhat) Force Ethers v5 stable path.
#    - The script will now *always* install 'hardhat@2.21.1' and
#      '@nomicfoundation/hardhat-toolbox@3.0.0' regardless of Node.js version.
#    - This forces the entire stack to use Ethers v5.
# 2. [FIX] (Hardhat) The script will now *always* use 'deploy-v5.js'.
#    - This resolves the complex dependency failures seen in the v7.2 log
#      (e.g., 'Compile failed') by locking to a known-stable combination.
# 3. [FIX] (Frontend) Fixed Parcel build: "Failed to resolve '@worldcoin/idkit-core/hashing'".
#    - The script now explicitly installs '@worldcoin/idkit-core' alongside
#      '@worldcoin/idkit' to ensure the sub-dependency is available.
# 4. [FIX] Includes all fixes from v7.1 (CI Email, Parcel 'main' key, etc.)
# 5. [FIX] Includes all fixes from v6.5-v7.0 (apt-get, .env, 502, etc.)
# =============================================================================
...
/* 🌐 ZeaZDev v7.3 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.3 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v7.3 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v7.3 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.3 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
EOF
  ...
  # --- [FIX v7.3] ---
  # Force Ethers v5 path regardless of Node.js version.
...
/* 🌐 ZeaZDev v7.3 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v7.3)</strong> — Deploy & Logs (Live)</header>
...
npm pkg set type="module" >/dev/null 2>&1 || true
# --- [FIX v7.3] ---
info "Installing frontend dependencies (pinning @worldcoin/idkit@1.1.2 and @worldcoin/idkit-core)..."
npm install --legacy-peer-deps react react-dom parcel ethers @worldcoin/idkit@1.1.2 @worldcoin/idkit-core >/dev/null 2>&1 || true
# --- [END FIX] ---
...
/* 🌐 ZeaZDev v7.3 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v7.3 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v7.3 $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v7.3 (Full Dep. Fix) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v7.3 (Hotfix 13) installation complete!"
