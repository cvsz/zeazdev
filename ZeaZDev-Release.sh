...
# 📦 Version: 1.0.0 — Build Stable Main WorldChain + MultiChain
...
# Example .env for ZeaZDev Release
MULTI_RPC_LIST=$MULTI_RPC_LIST
WORLD_APP_ID=$WORLD_APP_ID
PRIVATE_KEY=$PRIVATE_KEY
ETHERSCAN_API_KEY=$ETHERSCAN_API_KEY
...
(cd "$INSTALL_PATH" && node server.js &>/tmp/zeazdev_dashboard.log & echo $! > /tmp/zeazdev_dashboard.pid)

### =============== Git push deploy logs if configured ===============
info "Preparing deploy logs..."
LOGFILE="$LOG_DIR/deploy-$(date -u +%Y%m%dT%H%M%SZ).log"
echo "ZeaZDev Release Deploy Logs - $(date -u)" > "$LOGFILE"
...
if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_CHAT_ID" ]; then
  MSG="✅ ZeaZDev Release Multi-Deploy finished.\n"
  for f in "$RESULTS_DIR"/*.json; do
...
( cd "$FRONT_PATH" && npx parcel src/index.html --port 3000 &>/tmp/zeadev_parcel.log & echo $! > /tmp/zeazdev_parcel.pid )
sleep 4
...
info "ZeaZDev Release completed. Dashboard: http://<server-ip>:3000 (served by $INSTALL_PATH/server.js)"
