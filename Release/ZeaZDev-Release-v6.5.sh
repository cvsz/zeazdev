...
# 🔐 License: MIT
# =============================================================================
# HOTFIX v6.5 (Critical):
# 1. [FIX] (502 Bad Gateway) Changed Frontend (app.zeaz.dev) from a
#    proxy_pass (port 4000) to a direct static Nginx service.
#    - This is the correct production setup and eliminates the 502 error
#    - source from the 'zeazdev-front' service.
# 2. [FIX] Added 'npm run build' for the Frontend (Parcel) to create the
#    production 'dist' directory.
# 3. [FIX] Removed the 'zeazdev-front.service' (systemd), as it is no
#    longer needed. Nginx will serve the static files directly.
# 4. [FIX] Changed service restart order: Restart Node.js (dash) first,
#    *then* restart Nginx to prevent a 502 race condition.
# 5. [FIX] Hardened 'zeazdev-dash.service' with 'RestartSec=5' and
#    'ExecStart=/usr/bin/env node server.js'.
# 6. [FIX] (Security) Added 'autoindex off' (v6.4).
# 7. [FIX] (Input) Added 'MULTI_RPC_LIST' auto-correction (v6.2).
# =============================================================================
...
/* 🌐 ZeaZDev v6.5 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.5 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v6.5 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v6.5 (Deploy Script) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.5 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.5 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v6.5)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v6.5 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
function App(){
  ...
  return(<div style={{fontFamily:"Arial",padding:20}}>
    ...
    <IDKitWidget app_id={process.env.WORLD_APP_ID || "app_f6ca4506cc8d784843fcce064b387e0c"} action={ACTION} onSuccess={handleProof}
...
# --- [FIX v6.5] ---
# Changed Frontend config from proxy_pass to static file serving.
# This resolves the 502 Bad Gateway error.
...
server {
    ...
    # Point root to the Parcel build directory
    root $FRONT_PATH/dist;
    
    location / {
        # Try to find the file, otherwise fall back to index.html
        # This is required for React Router / SPA navigation
...
rm -f /etc/nginx/sites-enabled/zeazdev.conf /etc/nginx/sites-available/zeazdev.conf # Clean up old v6.2 config
...
# --- [FIX v6.5] ---
# Changed restart order to prevent 502 race condition.
...
echo "ZeaZDev v6.5 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  git -c user.email="ci@zeazdev.local" -c user.name="ZeaZDev CI" commit -m "Deploy v6.5 $(date -u)" >/dev/null 2>&1 || true
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v6.5 (502 Fix) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi

# --- 15. Cleanup ---
rm -rf "$TMP_PDIR"
info "ZeaZDev v6.5 (Hotfix 5) installation complete!"
