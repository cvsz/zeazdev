...
# 📦 Version: Release — v6.0 (Final Unified Build)
...
# 🔐 License: MIT
# =============================================================================
# นี่คือสคริปต์ One-Click Installer ที่รวมทุกอย่าง:
# (Hardhat Multi-Network + WorldID Airdrop + Reward Contract + Dashboard + Nginx + SSL + Telegram + Git Push)
# =============================================================================
...
: "${FRONT_DOMAIN:=${FRONT_DOMAIN:-app.zeaz.dev}}" # โดเมนหลักสำหรับ Frontend
: "${DASH_DOMAIN:=${DASH_DOMAIN:-dash.zeaz.dev}}" # โดเมนสำหรับ Dashboard
: "${WORLD_ROUTER_FALLBACK:=0x57f928158C3EE7CDad1e4D8642503c4D0201f611}" # (Fixed Router)
: "${MAX_PARALLEL:=2}" # ป้องกัน OOM Killer
: "${MIN_RAM_MB:=2000}" # RAM ขั้นต่ำก่อนสร้าง Swap
...
/* 🌐 ZeaZDev v6.0 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.0 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v6.0 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v6.0 (Deploy Script) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.0 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.0 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v6.0)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v6.0 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
import React,{useState}from"react";
import{createRoot}from"react-dom/client";
import{ethers}from"ethers";
import{IDKitWidget}from"@worldcoin/idkit";
import networks from "./networks.json";
const CONTRACT="0x...AirdropAddress"; // This will be auto-replaced by deploy.js
...
echo "ZeaZDev v6.0 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  git -c user.email="ci@zeazdev.local" -c user.name="ZeaZDev CI" commit -m "Deploy v6.0 $(date -u)" >/dev/null 2>&1 || true
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v6.0 Deploy Done ($NODE_VER). Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi

# --- 15. Cleanup ---
rm -rf "$TMP_DIR"
info "ZeaZDev v6.0 (Final Unified) installation complete!"
