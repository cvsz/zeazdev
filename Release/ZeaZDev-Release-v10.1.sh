...
# 📦 Version: Release — v10.1 (Stability Hotfix)
...
# 🔐 License: MIT
# =============================================================================
# UPGRADE v10.1 (Stability Revert & Fix):
# 1. [FIX] (Hardhat) Reverted Ethers v6 strategy. FORCING Ethers v5 path:
#    (Node 22 + Hardhat 2.21.1 + Toolbox 3.0.0 + deploy-v5.js).
#    This fixes the 'Job ... failed' errors from v10.0 logs.
# 2. [FIX] (Frontend) Upgraded @worldcoin/idkit to '@worldcoin/idkit@^1.3.6'
#    (based on repair script) and REMOVED Parcel Aliases.
# 3. [FIX] (Dashboard) Added '/api/health' endpoint (from repair script).
# 4. [FEAT] (Git) Includes v9.4 Git Push (Full Project).
# 5. [FEAT] (Backend) Includes v10.0 Backend/Vault/Gasless architecture.
# =============================================================================
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v10.1 (Stability Hotfix Engine)"
# --- [END FEAT] ---
...
/* 🌐 ZeaZDev v10.1 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v10.1 (Airdrop - Gasless) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v10.1 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v10.1 (Vault Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
# --- [FIX v10.1] ---
# Create Ethers v5 deploy script (for Node 22 + Hardhat 2.21.1)
cat > "$TMP_DIR/deploy-v5.js" <<'JS'
/* 🌐 ZeaZDev v10.1 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
async function main(){
  ...
  console.log(`Deployer ZEA balance: ${hre.ethers.utils.formatEther(balance)}`); // v5 syntax
  ...
  const airdropAmount = hre.ethers.utils.parseUnits("500000000", 18); // v5 syntax
  const rewardAmount = hre.ethers.utils.parseUnits("100000000", 18); // v5 syntax
...
/* 🌐 ZeaZDev v10.1 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v10.1 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v10.1 (API Backend Server) 👨‍💻 PHIPHAT PHOEMSUK */
import express from "express";
import http from "http";
import dotenv from "dotenv";
...
</head><body><header><strong>ZeaZDev Dashboard (v10.1)</strong> — Deploy & Logs (Live)</header>
...
# --- [FIX v10.1] ---
info "Installing frontend dependencies (pinning @worldcoin/idkit@^1.3.6)..."
info "Cleaning npm cache for Frontend..."
npm cache clean --force >/dev/null 2>&1 || true
npm install --legacy-peer-deps react react-dom parcel ethers @worldcoin/idkit@^1.3.6 >/dev/null 2>&1 || true
# --- [END FIX] ---
...
/* 🌐 ZeaZDev v10.1 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
function App() {
  ...
  return (
    <div style={{ fontFamily: "Arial", padding: 20 }}>
      <h2>ZeaZDev Airdrop (v10.1 Backend)</h2>
...
echo "ZeaZDev v10.1 Deploy Summary" > "$LOGFILE_FINAL"
...
GIGNORE
  ...
  git add .
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v10.1 (Stability Hotfix) $(date -u)" >/dev/null 2>&1 || true
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v10.1 (Stability Hotfix) Deploy Done. API: https://$API_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v10.1 (Stability Hotfix) installation complete!"
