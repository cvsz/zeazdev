...
# 📦 Version: Release — v9.4 (The Vault Integration)
...
# 🔐 License: MIT
# =============================================================================
# UPGRADE v9.4 (Stability Revert & Git Feature):
# 1. [FEAT] (Git) Upgraded Git Push to push the *entire* project, not just
#    logs. Added a .gitignore file to exclude .env, node_modules, etc.
# 2. [FIX] (Hardhat) Reverted Ethers v6 strategy. FORCING Ethers v5 path:
#    (Node 22 + Hardhat 2.21.1 + Toolbox 3.0.0 + deploy-v5.js).
# 3. [FIX] (Frontend) Reverted Alias strategy. Downgrading to stable
#    '@worldcoin/idkit@1.0.0' and removing 'idkit-core' and all aliases.
# 4. [FIX] (Health Check) Added '|| true' to all 'jq .' commands to
#    prevent 'set -e' from stopping the script if an API key is missing.
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v9.4 (Vault Integration Engine)"
# --- [END FEAT] ---
...
if [ -z "$MULTI_RPC_LIST" ] || [ -z "$WORLD_APP_ID" ] || [ -z "$PRIVATE_KEY" ]; then
  ...
  err "This failure is critical. The 'run.sh' (v9.4) safe parser should have caught this."
  exit 1
fi
...
/* 🌐 ZeaZDev v9.4 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.4 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v9.4 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.4 (Vault Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
# --- [FIX v9.4] ---
# We ONLY need the Ethers v5 script now, as we force this path.
cat > "$TMP_DIR/deploy-v5.js" <<'JS'
/* 🌐 ZeaZDev v9.4 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.4 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.4 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v9.4)</strong> — Deploy & Logs (Live)</header>
...
# --- [FIX v9.4] ---
info "Installing frontend dependencies (Downgrading @worldcoin/idkit@1.0.0)..."
info "Cleaning npm cache for Frontend..."
npm cache clean --force >/dev/null 2>&1 || true
npm install --legacy-peer-deps react react-dom parcel ethers @worldcoin/idkit@1.0.0 >/dev/null 2>&1 || true
# --- [END FIX] ---
...
/* 🌐 ZeaZDev v9.4 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v9.4 Deploy Summary" > "$LOGFILE_FINAL"
...
GIGNORE
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v9.4 (Full Project Sync) $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v9.4 (Stability Revert) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v9.4 (Vault Integration) installation complete!"
