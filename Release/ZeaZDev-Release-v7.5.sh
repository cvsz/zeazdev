...
# 📦 Version: Release — v7.5 (Hotfix 15)
...
# 🔐 License: MIT
# =============================================================================
# HOTFIX v7.5 (Critical Dependency & Interactive Fix):
# 1. [FIX] (Hardhat) Re-instated Node.js version check for Ethers v5/v6.
#    - Node < 22: Uses hardhat@latest (Ethers v6) + deploy-v6.js
#    - Node >= 22: Uses hardhat@2.21.1 (Ethers v5) + deploy-v5.js
# 2. [FIX] (Hardhat) Re-instated deploy-v6.js script.
# 3. [FIX] (Frontend) Upgraded @worldcoin/idkit to @latest.
# 4. [FIX] (Frontend) Added explicit install of @worldcoin/idkit-core.
# 5. [FIX] (Non-Interactive) Removed all 'read -p' prompts.
# 6. [FIX] (Validation) Added strict check for required ENV vars.
# =============================================================================
...
/* 🌐 ZeaZDev v7.5 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.5 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v7.5 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v7.5 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.5 (Deploy Script - Ethers v6 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
import hre from "hardhat";
import fs from "fs";
import dotenv from "dotenv";
dotenv.config();
...
/* 🌐 ZeaZDev v7.5 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.5 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v7.5)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v7.5 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v7.5 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v7.5 $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v7.5 (Non-Interactive) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v7.5 (Hotfix 15) installation complete!"
