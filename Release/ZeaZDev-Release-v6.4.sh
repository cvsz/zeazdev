...
# =============================================================================
# HOTFIX v6.4:
# 1. [FIX] (Security) Disable Nginx default site.
#    - Added 'rm -f /etc/nginx/sites-enabled/default' to prevent conflicts
#    - or directory listings from the default Nginx config.
# 2. [FIX] (Security) Explicitly disable directory listing.
#    - Added 'autoindex off;' to both 'dash.zeaz.dev' and 'app.zeaz.dev'
#    - Nginx server blocks to "protect index of".
# 3. [FIX] Included fixes from v6.3 (Nginx split configs, Parcel 'start' script).
# 4. [FIX] Included fixes from v6.2 (Auto-correct MULTI_RPC_LIST format).
# 5. [FIX] Included fixes from v6.1 (NODE22_MODE unbound variable).
# =============================================================================
...
/* 🌐 ZeaZDev v6.4 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.4 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v6.4 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v6.4 (Deploy Script) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.4 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.4 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
const app = express();
// Serve static files (index.html, app.js) from the Dashboard directory
...
</head><body><header><strong>ZeaZDev Dashboard (v6.4)</strong> — Deploy & Logs (Live)</header>
...
# --- [FIX v6.3] ---
info "Adding 'start' script to Frontend package.json"
npm pkg set scripts.start="parcel src/index.html" >/dev/null 2>&1 || true
# --- [END FIX] ---
mkdir -p "$FRONT_PATH/src"
...
/* 🌐 ZeaZDev v6.4 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
# --- [FIX v6.4] ---
# A. Nginx config (Split server blocks)
...
server {
    ...
    autoindex off; # <-- (FIX v6.4) Explicitly disable directory listing
...
rm -f /etc/nginx/sites-enabled/default # <-- (FIX v6.4) Remove default config
...
echo "ZeaZDev v6.4 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  git -c user.email="ci@zeazdev.local" -c user.name="ZeaZDev CI" commit -m "Deploy v6.4 $(date -u)" >/dev/null 2>&1 || true
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v6.4 Deploy Done ($NODE_VER). Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi

# --- 15. Cleanup ---
rm -rf "$TMP_DIR"
info "ZeaZDev v6.4 (Hotfix 4) installation complete!"
