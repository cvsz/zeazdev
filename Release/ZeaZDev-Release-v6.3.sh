...
# =============================================================================
# HOTFIX v6.3:
# 1. [FIX] Split Nginx config into separate 'server' blocks for 'DASH_DOMAIN'
#    (port 3000) and 'FRONT_DOMAIN' (port 4000) to route traffic correctly.
# 2. [FIX] Added 'start' script to 'Frontend/package.json' to ensure the
#    'zeazdev-front.service' (Parcel) can start correctly.
# 3. [FIX] Added auto-correction for 'MULTI_RPC_LIST' format (v6.2).
# 4. [FIX] Resolved 'unbound variable' for 'NODE22_MODE' (v6.1).
# =============================================================================
...
/* 🌐 ZeaZDev v6.3 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.3 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v6.3 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v6.3 (Deploy Script) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.3 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* �� ZeaZDev v6.3 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v6.3)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v6.3 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
# Note: 'npm run start' is now defined in package.json by [FIX v6.3]
...
echo "ZeaZDev v6.3 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  git -c user.email="ci@zeazdev.local" -c user.name="ZeaZDev CI" commit -m "Deploy v6.3 $(date -u)" >/dev/null 2>&1 || true
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v6.3 Deploy Done ($NODE_VER). Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi

# --- 15. Cleanup ---
rm -rf "$TMP_DIR"
info "ZeaZDev v6.3 (Hotfix 3) installation complete!"
