...
# 📦 Version: Release — v9.2 (The Vault Integration)
...
# 🔐 License: MIT
# =============================================================================
# UPGRADE v9.2 (Config & Frontend Fix):
# 1. [FEAT] (Config) This script is now an "Engine" called by 'run.sh'.
# 2. [FIX] (Config) 'run.sh' (v9.2) now safely parses .env, skipping
#    invalid identifiers (like GHCR-Push-Token).
# 3. [FIX] (Frontend) Fixed IDKit build error ("./hooks/useIDKit")
#    by aliasing the root '@worldcoin/idkit' package to its
#    compiled 'dist/index.mjs' file.
# 4. [FIX] (Hardhat) FORCED Ethers v5 path (Hardhat 2.21.1) (v8.0).
# 5. [FIX] (Security) Added 'ZeaZDevVault.sol' contract (v9.0).
# =============================================================================
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v9.2 (Vault Integration Engine)"
# --- [END FEAT] ---
...
if [ -z "$MULTI_RPC_LIST" ] || [ -z "$WORLD_APP_ID" ] || [ -z "$PRIVATE_KEY" ]; then
  ...
  err "This failure is critical. The 'run.sh' (v9.2) safe parser should have caught this."
  exit 1
fi
...
/* 🌐 ZeaZDev v9.2 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.2 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v9.2 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.2 (Vault Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.2 (Deploy Script - Vault Integration) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.2 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.2 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v9.2)</strong> — Deploy & Logs (Live)</header>
...
info "Applying Parcel Alias fix for IDKit (Root Alias)..."
npm pkg set 'alias."@worldcoin/idkit"'='@worldcoin/idkit/dist/index.mjs' >/dev/null 2>&1 || true
npm pkg set 'alias."@worldcoin/idkit-core"'='@worldcoin/idkit-core/dist/index.mjs' >/dev/null 2>&1 || true
# --- [END FIX] ---
...
/* 🌐 ZeaZDev v9.2 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v9.2 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v9.2 (Vault Integration) $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v9.2 (Vault Integrated) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v9.2 (Vault Integration) installation complete!"
