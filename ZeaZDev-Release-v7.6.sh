...
# 📦 Version: Release — v7.6 (Hotfix 16)
...
# 🔐 License: MIT
# =============================================================================
# HOTFIX v7.6 (Critical Dependency & Display Fix):
# 1. [FEAT] (Display) Added ASCII Art display on install.
# 2. [FIX] (Node.js) Installer now forces Node.js 22.x (LTS).
# 3. [FIX] (Hardhat) Force Ethers v5 path (Hardhat 2.21.1 / Toolbox 3.0.0).
# 4. [FIX] (Hardhat) Removed 'Auto-Heal HH801' step *AGAIN* (v7.4).
# 5. [FIX] (Frontend) Pinned @worldcoin/idkit AND @worldcoin/idkit-core
#    to '1.1.2' to resolve internal pathing errors.
# =============================================================================
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v7.6 (Hotfix 16)"
# --- [END FEAT] ---
...
/* 🌐 ZeaZDev v7.6 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.6 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v7.6 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v7.6 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.6 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.6 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v7.6)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v7.6 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v7.6 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v7.6 $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v7.6 (Node22/IDKit Fix) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v7.6 (Hotfix 16) installation complete!"
