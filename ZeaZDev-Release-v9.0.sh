...
# 📦 Version: Release — v9.0 (The Vault Integration)
...
# 🔐 License: MIT
# =============================================================================
# UPGRADE v9.0 (Vault Integration & Security):
# 1. [FEAT] (Security) Added 'ZeaZDevVault.sol' contract to act as the Treasury.
# 2. [FIX] (Deploy) 'deploy-v5.js' updated:
#    - Deploys the Vault.
#    - Transfers ALL remaining ZEA tokens to the Vault (not the deployer).
#    - Whitelists Airdrop/Reward contracts.
#    - Funds Airdrop/Reward contracts *from* the Vault.
# 3. [FIX] (UI) Dashboard UI updated to display the deployed Vault address.
# 4. [FIX] (Hardhat) FORCED Ethers v5 path (Hardhat 2.21.1 / Toolbox 3.0.0).
# 5. [FIX] (Frontend) Fixed IDKit build error ("./hooks/useIDKit") via 'alias'.
# =============================================================================
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v9.0 (Vault Integration)"
# --- [END FEAT] ---
...
/* 🌐 ZeaZDev v9.0 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.0 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v9.0 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.0 (Vault Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
# --- [FIX v8.0] ---
# We ONLY need the Ethers v5 script now, as we force Node.js 22.
cat > "$TMP_DIR/deploy-v5.js" <<'JS'
/* 🌐 ZeaZDev v9.0 (Deploy Script - Vault Integration) 👨‍💻 PHIPHAT PHOEMSUK */
...
async function main(){
  ...
  await zea.deployed(); // v5 syntax
  const zeaAddr = zea.address; // v5 syntax
  ...
  await airdrop.deployed(); // v5 syntax
  const airdropAddr = airdrop.address; // v5 syntax
  ...
  await reward.deployed(); // v5 syntax
  const rewardAddr = reward.address; // v5 syntax
  ...
  console.log(`Deployer ZEA balance: ${hre.ethers.utils.formatEther(balance)}`);
  await zea.transfer(vaultAddr, balance);
  console.log(`Transferred ${hre.ethers.utils.formatEther(balance)} ZEA to Vault.`);
  ...
  const airdropAmount = hre.ethers.utils.parseUnits("500000000", 18); // 500M
  const rewardAmount = hre.ethers.utils.parseUnits("100000000", 18); // 100M
  ...
  const result = {
    ...
    airdrop: airdropAddr,
    reward: rewardAddr,
    vault: vaultAddr // <-- Added Vault address
  };
...
/* 🌐 ZeaZDev v9.0 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
EOF
  ...
  # --- [FIX v8.0] ---
  # Force Ethers v5 path (since we forced Node 22).
  info "[$net] Forcing Ethers v5 path: Using Hardhat v2.21.1 & Toolbox v3.0.0"
  ...
  cp "$TMP_DIR/deploy-v5.js" "$ws/scripts/deploy.js"
  info "[$net] NPM Dependencies (Depth 1):"
  npm list --depth=1 || true
  # --- [END FIX] ---
  
  # E. Auto-Heal HH801 (REMOVED in v7.4)
...
/* 🌐 ZeaZDev v9.0 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v9.0)</strong> — Deploy & Logs (Live)</header>
...
# --- [FIX v8.0] ---
info "Installing frontend dependencies (pinning @worldcoin/idkit@1.1.2 AND @worldcoin/idkit-core@1.1.2)..."
info "Cleaning npm cache for Frontend..."
npm cache clean --force >/dev/null 2>&1 || true
npm install --legacy-peer-deps react react-dom parcel ethers @worldcoin/idkit@1.1.2 @worldcoin/idkit-core@1.1.2 >/dev/null 2>&1 || true
info "Applying Parcel Alias fix for IDKit..."
...
/* 🌐 ZeaZDev v9.0 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v9.0 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v9.0 (Vault Integration) $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v9.0 (Vault Integrated) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi

# --- [FEAT v8.0] 15. API Health Check ---
...
curl -s -X GET "https://api.etherscan.io/api/v2/accounts/0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045/balance?apikey=${ETHERSCAN_API_KEY}" | jq .
# 3. Worldcoin (docs.world.org)
info "[3/4] Querying Worldcoin JWKS (id.worldcoin.org)..."
curl -s -L "https://id.worldcoin.org/jwks.json" | jq '.keys[0] | {kty: .kty, kid: .kid}'
# 4. GitHub Repos (MetaMask, Ethereum, Worldcoin Mini Apps)
info "[4/4] Querying GitHub Repos (Latest Releases)..."
curl -s -L "https://api.github.com/repos/worldcoin/idkit/releases/latest" | jq -r '(.tag_name + " (worldcoin/idkit)")'
curl -s -L "https://api.github.com/repos/MetaMask/metamask-extension/releases/latest" | jq -r '(.tag_name + " (MetaMask/metamask-extension)")'
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v9.0 (Vault Integration) installation complete!"
