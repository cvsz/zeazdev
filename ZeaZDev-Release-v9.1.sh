...
# 📦 Version: Release — v9.1 (The Vault Integration)
...
# 🔐 License: MIT
# =============================================================================
# UPGRADE v9.1 (Config Standardization):
# 1. [FEAT] (Config) This script is now an "Engine" called by 'run.sh'.
# 2. [FIX] (Config) Removed 'cat > .env' logic.
# 3. [FIX] (Config) Added 'cp ./.env /opt/ZeaZDev/.env' to copy the
#    root .env file for the systemd service to use.
# 4. [FIX] (Security) Added 'ZeaZDevVault.sol' contract (v9.0).
# 5. [FIX] (Hardhat) FORCED Ethers v5 path (Hardhat 2.21.1 / Toolbox 3.0.0) (v8.0).
# 6. [FIX] (Frontend) Fixed IDKit build error ("./hooks/useIDKit") via 'alias' (v8.0).
# =============================================================================
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v9.1 (Vault Integration Engine)"
# --- [END FEAT] ---
...
/* 🌐 ZeaZDev v9.1 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.1 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v9.1 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.1 (Vault Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.1 (Deploy Script - Vault Integration) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.1 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.1 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v9.1)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v9.1 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v9.1 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v9.1 (Vault Integration) $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v9.1 (Vault Integrated) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v9.1 (Vault Integration) installation complete!"
