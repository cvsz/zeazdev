...
# 📦 Version: Release — v7.2 (Hotfix 12)
...
# 🔐 License: MIT
# =============================================================================
# HOTFIX v7.2 (Critical Dependency Fix):
# 1. [FIX] (Hardhat) Resolved Ethers v5/v6 conflict permanently.
#    - The script now carries TWO deploy scripts: 'deploy-v5.js' and 'deploy-v6.js'.
#    - If Node.js >= 22, it installs Hardhat Toolbox v3 (Ethers v5) and uses 'deploy-v5.js'.
#    - If Node.js < 22, it installs Hardhat Toolbox latest (Ethers v6) and uses 'deploy-v6.js'.
#    - This fixes all 'Compile failed' errors from the v7.1 log.
# 2. [FIX] (Frontend) Fixed Parcel build error: "Failed to resolve './hooks/useIDKit'".
#    - Pinned '@worldcoin/idkit' to version '1.1.2', a known stable version.
# 3. [FIX] Includes all fixes from v7.1 (CI Email, Parcel 'main' key, etc.)
# 4. [FIX] Includes all fixes from v6.5-v7.0 (apt-get, .env, 502, etc.)
# =============================================================================
...
/* 🌐 ZeaZDev v7.2 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v7.2 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v7.2 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
# --- 5. Write Deploy Script Templates ---
# --- [FIX v7.2] ---
# Create BOTH Ethers v5 and Ethers v6 deploy scripts.
# We will copy the correct one based on Node.js version.

# Script 1: Ethers v5 (for Node 22+ / Toolbox v3)
cat > "$TMP_DIR/deploy-v5.js" <<'JS'
/* 🌐 ZeaZDev v7.2 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
# Script 2: Ethers v6 (for Node < 22 / Toolbox v5+)
cat > "$TMP_DIR/deploy-v6.js" <<'JS'
/* 🌐 ZeaZDev v7.2 (Deploy Script - Ethers v6 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
import hre from "hardhat";
import fs from "fs";
import dotenv from "dotenv";
dotenv.config();
async function main(){
  ...
  console.log("Reward:", rewardAddr);
  console.log("Funding contracts...");
  await zea.transfer(airdropAddr, hre.ethers.parseUnits("500000000", 18)); // v6 syntax
  await zea.transfer(rewardAddr, hre.ethers.parseUnits("100000000", 18)); // v6 syntax
  const result = {
    network: process.env.NETWORK,
    token: zeaAddr,
    airdrop: airdropAddr,
...
/* 🌐 ZeaZDev v7.2 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
EOF
  ...
  # --- [FIX v7.2] ---
  # Install correct Hardhat/Toolbox and copy correct deploy script
  if [ "$NODE_MAJOR" -ge 22 ]; then
    info "[$net] Node.js $NODE_VER >= 22. Using Hardhat v2.21.1 (Ethers v5)"
    ...
    info "[$net] Node.js $NODE_VER < 22. Using Hardhat latest (Ethers v6)"
...
/* 🌐 ZeaZDev v7.2 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v7.2)</strong> — Deploy & Logs (Live)</header>
...
npm pkg set type="module" >/dev/null 2>&1 || true
# --- [FIX v7.2] ---
info "Installing frontend dependencies (pinning @worldcoin/idkit@1.1.2)..."
npm install --legacy-peer-deps react react-dom parcel ethers @worldcoin/idkit@1.1.2 >/dev/null 2>&1 || true
# --- [END FIX] ---
...
/* 🌐 ZeaZDev v7.2 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v7.2 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v7.2 $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v7.2 (Dep. Fix) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v7.2 (Hotfix 12) installation complete!"
