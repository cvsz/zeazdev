...
# =============================================================================
# HOTFIX v6.1:
# 1. [FIX] Resolved 'unbound variable' error for 'NODE22_MODE'.
# 2. [FIX] Added validation for MULTI_RPC_LIST input format (net=rpc).
# =============================================================================
...
# --- [FIX v6.1] ---
# Initialize NODE22_MODE based on major version
NODE22_MODE=false
...
/* 🌐 ZeaZDev v6.1 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.1 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v6.1 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v6.1 (Deploy Script) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.1 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v6.1 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v6.1)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v6.1 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v6.1 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  git -c user.email="ci@zeazdev.local" -c user.name="ZeaZDev CI" commit -m "Deploy v6.1 $(date -u)" >/dev/null 2>&1 || true
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v6.1 Deploy Done ($NODE_VER). Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi

# --- 15. Cleanup ---
rm -rf "$TMP_DIR"
info "ZeaZDev v6.1 (Hotfix 1) installation complete!"
