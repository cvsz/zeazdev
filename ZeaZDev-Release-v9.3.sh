...
# 📦 Version: Release — v9.3 (The Vault Integration)
...
# 🔐 License: MIT
# =============================================================================
# UPGRADE v9.3 (Final Dependency Fix):
# 1. [FIX] (Hardhat) REVERTED Ethers v5 strategy.
#    - Installer now uses hardhat@latest + toolbox@latest (Ethers v6).
# 2. [FIX] (Hardhat) Added 'deploy-v6.js' (Ethers v6 syntax).
# 3. [FIX] (Frontend) Reverted to 'alias' (v8.0) strategy.
#    - Aliasing specific internal paths (e.g., '@worldcoin/idkit-core/hashing').
# 4. [FIX] (Health Check) Added '|| true' to all 'jq' commands
#    to prevent 'set -e' exit if API_KEY is missing.
# 5. [FIX] (Config) 'run.sh' (v9.2) now safely parses .env.
# =============================================================================
...
EOF
echo -e "${RESET}"
info "Starting ZeaZDev Unified Installer v9.3 (Vault Integration Engine)"
# --- [END FEAT] ---
...
/* 🌐 ZeaZDev v9.3 (ZEA Token) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.3 (Airdrop) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
/* 🌐 ZeaZDev v9.3 (Reward Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.3 (Vault Contract) 👨‍💻 PHIPHAT PHOEMSUK */
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
/* 🌐 ZeaZDev v9.3 (Deploy Script - Ethers v5 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
...
/* 🌐 ZeaZDev v9.3 (Deploy Script - Ethers v6 Syntax) 👨‍💻 PHIPHAT PHOEMSUK */
import hre from "hardhat";
import fs from "fs";
import dotenv from "dotenv";
dotenv.config();
async function main(){
  ...
  await zea.transfer(vaultAddr, balance);
  console.log(`Transferred ${hre.ethers.formatEther(balance)} ZEA to Vault.`); // v6 syntax
  console.log("Whitelisting contracts in Vault...");
  await vault.addWhitelist(airdropAddr);
  await vault.addWhitelist(rewardAddr);
  console.log("Airdrop and Reward contracts whitelisted.");
...
/* 🌐 ZeaZDev v9.3 (Hardhat Config) 👨‍💻 PHIPHAT PHOEMSUK */
...
EOF
  ...
  # --- [FIX v9.3] ---
  # Re-instating v7.5 logic. This is the correct path.
...
/* 🌐 ZeaZDev v9.3 (Dashboard Server) 👨‍💻 PHIPHAT PHOEMSUK */
...
</head><body><header><strong>ZeaZDev Dashboard (v9.3)</strong> — Deploy & Logs (Live)</header>
...
/* 🌐 ZeaZDev v9.3 (Frontend) 👨‍💻 PHIPHAT PHOEMSUK */
...
echo "ZeaZDev v9.3 Deploy Summary" > "$LOGFILE_FINAL"
...
if [ -n "$GIT_LOG_REPO" ]; then
  ...
  git add .
  # --- [FIX v7.1] ---
  git -c user.email="ci@zeaz.dev" -c user.name="ZeaZDev CI" commit -m "Deploy v9.3 (Vault Integration) $(date -u)" >/dev/null 2>&1 || true
  # --- [END FIX] ---
...
if [ -n "$TELEGRAM_BOT_TOKEN" ]; then
  ...
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
    ...
    -d text="✅ ZeaZDev v9.3 (Vault Integrated) Deploy Done. Dashboard: https://$DASH_DOMAIN | Frontend: https://$FRONT_DOMAIN ... Summary: $SUMMARY" >/dev/null 2>&1 || true
fi
...
rm -rf "$TMP_DIR"
# --- [END FIX] ---
info "ZeaZDev v9.3 (Vault Integration) installation complete!"
